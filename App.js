      ...
      <View style={styles.textView}>
        <View style={styles.textGroupView}>
          ...
          <View style={styles.circleAndStatusView}>
            {setStatusCircle({ item })}
            <Text style={styles.status}> {item.status} - {item.species}</Text>
          </View>

        </View>
        <View style={styles.textGroupView}>
          <Text style={styles.lastKnownLoc}>Last known location:</Text>
          <Text style={styles.locationName}>{item.location.name}</Text>
        </View>
        <View style={styles.textGroupView}>
          {getFirstSeen(item.episode[0])}
          <Text style={styles.lastKnownLoc}>First Seen in:</Text>
          <Text style={styles.locationName}>{firstSeen[item.id - 1]}</Text>
        </View>
      </View>
    </View>
  );
